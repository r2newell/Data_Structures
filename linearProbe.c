...
bool insert(hashTable *dictionary, node *item, unsigned long long size)
{
    ...
    int counter = 1; 
    while((dictionary->table[index]) != NULL && (dictionary->table[index]->status) != 1)
    {
...
node *search(hashTable *dictionary, const char *key, unsigned long long size)
{
    ...
    while((dictionary->table[index]) != NULL && (dictionary->table[index]->status) != 1)
    {
        if(counter == size)
            break; 
        if(strcmp(key, dictionary->table[index]->key) == 0)
            break; 
        index = (index + counter) % size;
	   counter++; 
    }
    ...
    if(counter != size)
        if(dictionary->table[index] && dictionary->table[index]->status == 0)
