...
int main()
{
    ...
    while(counter < 2)
    {
        ...
        option = getchar(); 
        if(option == 'y')
                addEdge(&flights, name[0], name[1], weight); 
        else
        {
            addEdge(&flights, name[0], name[1], weight); 
            addEdge(&flights, name[1], name[0], weight); 
        }
        counter++; 
    }

    display_Graph(&flights); 
    depth_First(&flights); 
    counter = 0; 
    printf("\n\tStart\t End");
    while(counter < flights.current)
    {
        printf("\n%s\t%d\t%d",flights.name[counter], start[counter], end[counter]); 
        counter++; 
    }

    breadth_First(&flights, 0); 
    
    counter = 0;
    printf("\nBFS\n");
    while(counter < flights.current)
    {
        printf("\n%s\t%d\t%.lf",flights.name[counter], parent[counter], distance[counter]);
        counter++;
    }
    
    if(bipartite(&flights))
        printf("\nThe graph is bipartite.\n");
    else
        printf("\nThe graph is not bipartite");
    
    connectedComponents(&flights);
/*    int source = get_Vertex(&flights, "s"); 
    int destination = get_Vertex(&flights, "x"); 

    bellman_Ford(&flights, "s"); 
    node *head;
    deque *path = getPath(source, destination);  
    ...
    while(head)
    {
        printf("%s: %.lf, ", flights.name[head->origin], distance[head->origin]);
        head = head->next; 
    } 
    printf("\n");
    deque_clear(path); 
    ...
    printf("\nTopological Sort: "); 
    deque *topological = topologicalSort(&flights); 
    head = topological->head; 

    while(head)
    {
        printf(" %s", flights.name[head->origin]); 
        head = head->next; 
    }

    deque_clear(topological);
    free(topological); 
    strongly_Connected(&flights);
   */ 
    clear_Graph(&flights); 
    printf("\n");
    return 0; 
