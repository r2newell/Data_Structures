...
void updateMinMax(Tree *redBlack, const node *root)
{
	if(redBlack->minimum == NULL && redBlack->maximum == NULL)
		   redBlack->minimum = redBlack->maximum = (node *)root;
	else if(root->key < redBlack->minimum->key)	
		   redBlack->minimum = (node *)root; 
	else if(root->key > redBlack->maximum->key)
		   redBlack->maximum = (node *)root; 
	else if(redBlack->minimum == root)
		   redBlack->minimum = redBlack->minimum->right? redBlack->minimum->right : redBlack->minimum->parent; 
	else if(redBlack->maximum == root)
		   redBlack->maximum = redBlack->maximum->left? redBlack->maximum->left : redBlack->maximum->parent; 
}
